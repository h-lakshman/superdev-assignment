...
describe("Solana Fellowship API", () => {
  ...
  test("POST /send/token should create valid SPL token transfer instruction", async () => {
    ...
    let sourceAta = await getAssociatedTokenAddress(mintKeypair.publicKey, ownerKeypair.publicKey);
    ...
    // Verify account structure
    const accounts = res.data.data.accounts;
    expect(accounts[0].pubkey).toBe(sourceAta.toString());           // Source ATA (owner's ATA for mint)
    expect(accounts[1].pubkey).toBe(destinationAta.toString());      // Destination ATA
    expect(accounts[2].pubkey).toBe(ownerKeypair.publicKey.toString()); // Owner authority
